...
pub fn decrefRcPtrC(bytes_or_null: ?[*]isize, alignment: u32) callconv(.C) void {
...
pub fn decrefCheckNullC(bytes_or_null: ?[*]u8, alignment: u32) callconv(.C) void {
...
pub fn decrefDataPtrC(bytes_or_null: ?[*]u8, alignment: u32) callconv(.C) void {
...
pub fn increfDataPtrC(bytes_or_null: ?[*]u8, inc_amount: isize) callconv(.C) void {
    const bytes = bytes_or_null orelse return;
...
pub fn freeDataPtrC(bytes_or_null: ?[*]u8, alignment: u32) callconv(.C) void {
    const bytes = bytes_or_null orelse return;
...
pub fn freeRcPtrC(bytes_or_null: ?[*]isize, alignment: u32) callconv(.C) void {
    const bytes = bytes_or_null orelse return;
    return free_ptr_to_refcount(bytes, alignment);
}

pub fn decref(bytes_or_null: ?[*]u8, data_bytes: usize, alignment: u32) void {
    if (data_bytes == 0) {
        return;
    }
...
inline fn free_ptr_to_refcount(refcount_ptr: [*]isize, alignment: u32) void {
    if (RC_TYPE == Refcount.none) return;
...
pub fn allocateWithRefcountC(data_bytes: usize, element_alignment: u32) callconv(.C) [*]u8 {
    return allocateWithRefcount(data_bytes, element_alignment);
}

pub fn allocateWithRefcount(data_bytes: usize, element_alignment: u32) [*]u8 {
