...
const SMALL_STRING_SIZE = @sizeOf(Str);
...
pub const Str = extern struct {
    ...
    pub fn init(bytes_ptr: [*]const u8, length: usize) Str {
        var result = Str.allocate(length);
    ...
    pub fn fromSubListUnsafe(list: roc.List, start: usize, count: usize, update_mode: UpdateMode) Str {
    ...
    pub fn isSeamlessSlice(self: Str) bool {
        return !self.isSmallStr() and @as(isize, @bitCast(self.str_len)) < 0;
    }

    pub fn fromSlice(slice: []const u8) Str {
        return Str.init(slice.ptr, slice.len);
    }

    fn allocateBig(length: usize, capacity: usize) Str {
    ...
    // allocate space for a (big or small) Str, but put nothing in it yet.
    // May have a larger capacity than the length.
    pub fn allocate(length: usize) Str {
        const element_width = 1;
        const result_is_big = length >= SMALL_STRING_SIZE;

        if (result_is_big) {
            const capacity = utils.calculateCapacity(0, length, element_width);
            return Str.allocateBig(length, capacity);
        } else {
            var string = Str.empty();

            string.asU8ptrMut()[@sizeOf(Str) - 1] = @as(u8, @intCast(length)) | 0b1000_0000;
    ...
    pub fn allocateExact(length: usize) Str {
        const result_is_big = length >= SMALL_STRING_SIZE;

        if (result_is_big) {
            return Str.allocateBig(length, length);
        } else {
            var string = Str.empty();
    ...
    pub fn seamlessSliceMask(self: Str) usize {
    ...
    pub fn getRefcountPtr(self: Str) ?[*]u8 {
    ...
    pub fn incref(self: Str, n: usize) void {
        if (!self.isSmallStr()) {
    ...
    pub fn decref(self: Str) void {
        if (!self.isSmallStr()) {
            utils.decref(self.getRefcountPtr(), self.str_capacity, Str.alignment);
        }
    }
    ...
    pub fn clone(str: Str) Str {
        if (str.isSmallStr()) {
            // just return the bytes
            return str;
        }

        const new_str = Str.allocateBig(str.str_len, str.str_len);
    ...
    pub fn reallocate(
        ...
        if (self.str_bytes) |source_ptr| {
            ...
            return Str{ .str_bytes = new_source, .str_len = new_length, .str_capacity = new_capacity };
        }
        return self.reallocateFresh(new_length);
    }
    ...
    fn reallocateFresh(
        ...
        if (result_is_big) {
            ...
            var result = Str.allocateBig(new_length, capacity);
            ...
            dest_ptr[@sizeOf(Str) - 1] = @as(u8, @intCast(new_length)) | 0b1000_0000;
    ...
    pub fn isSmallStr(self: Str) bool {
        return @as(isize, @bitCast(self.str_capacity)) < 0;
    }

    test "isSmallStr: returns true for empty string" {
        try expect(isSmallStr(Str.empty()));
    }

    fn asArray(self: Str) [@sizeOf(Str)]u8 {
        const as_ptr = @as([*]const u8, @ptrCast(&self));
        const slice = as_ptr[0..@sizeOf(Str)];
    ...
    pub fn len(self: Str) usize {
        if (self.isSmallStr()) {
            return self.asArray()[@sizeOf(Str) - 1] ^ 0b1000_0000;
        } else {
            return self.str_len & (~SEAMLESS_SLICE_BIT);
        }
    }

    pub fn setLen(self: *Str, length: usize) void {
        if (self.isSmallStr()) {
            self.asU8ptrMut()[@sizeOf(Str) - 1] = @as(u8, @intCast(length)) | 0b1000_0000;
        } else {
            self.str_len = length | (SEAMLESS_SLICE_BIT & self.str_len);
        }
    }

    pub fn getCapacity(self: Str) usize {
        if (self.isSmallStr()) {
    ...
    pub fn getUnchecked(self: Str, index: usize) u8 {
        if (self.isSmallStr()) {
    ...
    pub fn isUnique(self: Str) bool {
        ...
        return @call(.always_inline, Str.isRefcountOne, .{self});
    }

    fn isRefcountOne(self: Str) bool {
        return self.refcountMachine() == utils.REFCOUNT_ONE;
    }

    fn refcountMachine(self: Str) usize {
    ...
    fn refcountHuman(self: Str) usize {
    ...
    pub fn asSliceWithCapacity(self: *const Str) []const u8 {
        return self.asU8ptr()[0..self.getCapacity()];
    }

    pub fn asSliceWithCapacityMut(self: *Str) []u8 {
        return self.asU8ptrMut()[0..self.getCapacity()];
    }

    pub fn asU8ptr(self: *const Str) [*]const u8 {
        if (self.isSmallStr()) {
    ...
    pub fn memcpy(self: Str, dest: [*]u8) void {
        const src = self.asU8ptr();
        @memcpy(dest[0..self.len()], src[0..self.len()]);
    }

    test "Str.eq: small, equal" {
        const str1_len = 3;
        var str1: [str1_len]u8 = "abc".*;
        const str1_ptr: [*]u8 = &str1;
        var roc_str1 = Str.init(str1_ptr, str1_len);

        const str2_len = 3;
        var str2: [str2_len]u8 = "abc".*;
        const str2_ptr: [*]u8 = &str2;
        var roc_str2 = Str.init(str2_ptr, str2_len);
    ...
    test "Str.eq: small, not equal, different length" {
        const str1_len = 4;
    ...
    test "Str.eq: small, not equal, same length" {
        const str1_len = 3;
    ...
    test "Str.eq: large, equal" {
        const content = "012345678901234567890123456789";
        const roc_str1 = Str.init(content, content.len);
        const roc_str2 = Str.init(content, content.len);
    ...
    test "Str.eq: large, different lengths, unequal" {
        const content1 = "012345678901234567890123456789";
        const roc_str1 = Str.init(content1, content1.len);
        const content2 = "012345678901234567890";
        const roc_str2 = Str.init(content2, content2.len);
    ...
    test "Str.eq: large, different content, unequal" {
        const content1 = "012345678901234567890123456789!!";
        const roc_str1 = Str.init(content1, content1.len);
    ...
    test "Str.eq: large, garbage after end, equal" {
        const content = "012345678901234567890123456789";
        const roc_str1 = Str.init(content, content.len);
        const roc_str2 = Str.init(content, content.len);
...
pub fn init(bytes_ptr: [*]const u8, length: usize) callconv(.C) Str {
    return @call(.always_inline, Str.init, .{ bytes_ptr, length });
}

// Str.equal
pub fn strEqual(self: Str, other: Str) callconv(.C) bool {
    return self.eq(other);
}

// Str.numberOfBytes
pub fn strNumberOfBytes(string: Str) callconv(.C) usize {
    return string.len();
}

// Str.toScalars
pub fn strToScalarsC(str: Str) callconv(.C) roc.List {
    return @call(.always_inline, strToScalars, .{str});
}

fn strToScalars(string: Str) callconv(.C) roc.List {
    const str_len = string.len();

    if (str_len == 0) {
        return roc.List.empty();
    }
    ...
    var answer = roc.List.allocate(@alignOf(u32), capacity, @sizeOf(u32));

    // `orelse unreachable` is fine here, because we already did an early
    // return to verify the string was nonempty.
    const answer_elems = answer.elements(u32) orelse unreachable;
...
// Given a non-empty Str, and a src_index byte index into that string,
...
inline fn writeNextScalar(non_empty_string: Str, src_index: usize, dest: [*]u32, dest_index: usize) usize {
...
test "strToScalars: empty string" {
    const str = Str.fromSlice("");
    defer Str.decref(str);

    const expected = roc.List.empty();
    const actual = strToScalars(str);
    defer actual.decref(@sizeOf(u32));

    try expect(roc.List.eql(actual, expected));
}

test "strToScalars: One ASCII char" {
    const str = Str.fromSlice("R");
    defer Str.decref(str);

    const expected_array = [_]u32{82};
    const expected = roc.List.fromSlice(u32, expected_array[0..expected_array.len]);
...
fn strFromIntHelp(comptime T: type, int: T) Str {
...
fn strFromFloatHelp(comptime T: type, float: T) Str {
...
// Str.split
pub fn strSplit(string: Str, delimiter: Str) callconv(.C) roc.List {
    const segment_count = countSegments(string, delimiter);
    const list = roc.List.allocate(@alignOf(Str), segment_count, @sizeOf(Str));

    if (list.bytes) |bytes| {
        const strings = @as([*]Str, @ptrCast(@alignCast(bytes)));
...
fn initFromSmallStr(slice_bytes: [*]u8, len: usize, _: usize) Str {
    return Str.init(slice_bytes, len);
}

// The ref_ptr must already be shifted to be ready for storing in a seamless slice.
fn initFromBigStr(slice_bytes: [*]u8, len: usize, ref_ptr: usize) Str {
...
fn strSplitHelp(array: [*]Str, string: Str, delimiter: Str) void {
...
test "strSplitHelp: empty delimiter" {
    // Str.split "abc" "" == ["abc"]
    const str_arr = "abc";
    const str = Str.init(str_arr, str_arr.len);

    const delimiter_arr = "";
    const delimiter = Str.init(delimiter_arr, delimiter_arr.len);

    var array: [1]Str = undefined;
    const array_ptr: [*]Str = &array;

    strSplitHelp(array_ptr, str, delimiter);

    const expected = [1]Str{str};
...
test "strSplitHelp: empty start" {
    ...
    var array = [_]Str{undefined} ** array_len;

    const array_ptr: [*]Str = &array;

    strSplitHelp(array_ptr, str, delimiter);

    const one = Str.init("a", 1);

    const expected = [2]Str{ Str.empty(), one };
...
test "strSplitHelp: empty end" {
    ...
    const one = Str.init("1", 1);
    const two = Str.init("2", 1);

    const expected = [3]Str{
        one, two, Str.empty(),
    };
...
test "strSplitHelp: string equals delimiter" {
    ...
    const str_delimiter = Str.init(str_delimiter_arr, str_delimiter_arr.len);

    const array_len: usize = 2;
    var array: [array_len]Str = [_]Str{
        undefined,
        undefined,
    };
    const array_ptr: [*]Str = &array;
    ...
    const expected = [_]Str{ Str.empty(), Str.empty() };
...
test "strSplitHelp: delimiter on sides" {
    ...
    const ghi_arr = "ghi";
    const ghi = Str.init(ghi_arr, ghi_arr.len);

    const expected = [_]Str{ Str.empty(), ghi, Str.empty() };
...
test "strSplitHelp: three pieces" {
    ...
    const array_len: usize = 3;
    var array: [array_len]Str = undefined;
    const array_ptr: [*]Str = &array;

    strSplitHelp(array_ptr, str, delimiter);

    const a = Str.init("a", 1);
    const b = Str.init("b", 1);
    const c = Str.init("c", 1);

    const expected_array = [_]Str{ a, b, c };
...
test "strSplitHelp: overlapping delimiter 1" {
    ...
    var array: [2]Str = undefined;
    const array_ptr: [*]Str = &array;

    strSplitHelp(array_ptr, str, delimiter);

    const expected = [_]Str{
        Str.empty(),
        Str.init("a", 1),
    };
...
test "strSplitHelp: overlapping delimiter 2" {
    ...
    var array: [3]Str = undefined;
    const array_ptr: [*]Str = &array;

    strSplitHelp(array_ptr, str, delimiter);

    const expected = [_]Str{Str.empty()} ** 3;
...
pub fn countSegments(string: Str, delimiter: Str) callconv(.C) usize {
    const str_bytes = string.asU8ptr();
    const str_len = string.len();
...
test "countSegments: overlapping delimiter 1" {
    // Str.split "aaa" "aa" == ["", "a"]
    const segments_count = countSegments(Str.init("aaa", 3), Str.init("aa", 2));
...
test "countSegments: overlapping delimiter 2" {
    // Str.split "aaa" "aa" == ["", "a"]
    const segments_count = countSegments(Str.init("aaaa", 4), Str.init("aa", 2));

    try expectEqual(segments_count, 3);
}

// // Str.countGraphemeClusters
// pub fn countGraphemeClusters(string: Str) callconv(.C) usize {
//     if (string.isEmpty()) {
//         return 0;
//     }
...
// // Str.graphemes
// pub fn strGraphemes(roc_str: Str) callconv(.C) roc.List {
...
//     var result = roc.List.allocate(@alignOf(Str), countGraphemeClusters(roc_str), @sizeOf(Str));
//     const graphemes = result.elements(Str) orelse return result;
...
//     const rocstr = Str.fromSlice(input);
//     defer rocstr.decref();
...
//     const elems = graphemes.elements(Str) orelse unreachable;
//     for (expected, 0..) |g, i| {
...
pub fn countUtf8Bytes(string: Str) callconv(.C) usize {
    return string.len();
}

pub fn isEmpty(string: Str) callconv(.C) bool {
    return string.isEmpty();
}

pub fn getCapacity(string: Str) callconv(.C) usize {
    return string.getCapacity();
}

pub fn substringUnsafe(string: Str, start: usize, length: usize) callconv(.C) Str {
    const slice = string.asSlice()[start .. start + length];
    return Str.fromSlice(slice);
}

pub fn getUnsafe(string: Str, index: usize) callconv(.C) u8 {
    return string.getUnchecked(index);
}

test "substringUnsafe: start" {
    const str = Str.fromSlice("abcdef");
    defer str.decref();

    const expected = Str.fromSlice("abc");
    defer expected.decref();

    const actual = substringUnsafe(str, 0, 3);

    try expect(Str.eq(actual, expected));
}
...
test "substringUnsafe: middle" {
    ...
    const expected = Str.fromSlice("bcd");
    defer expected.decref();
...
test "substringUnsafe: end" {
    const str = Str.fromSlice("a string so long it is heap-allocated");
    defer str.decref();

    const expected = Str.fromSlice("heap-allocated");
    defer expected.decref();
...
// Str.startsWith
pub fn startsWith(string: Str, prefix: Str) callconv(.C) bool {
    const bytes_len = string.len();
    const bytes_ptr = string.asU8ptr();
...
// Str.repeat
pub fn repeat(string: Str, count: usize) callconv(.C) Str {
    const bytes_len = string.len();
    const bytes_ptr = string.asU8ptr();

    var ret_string = Str.allocate(count * bytes_len);
...
pub fn startsWithScalar(string: Str, prefix: u32) callconv(.C) bool {
    const str_len = string.len();
...
test "startsWithScalar: empty string" {
    const whole = Str.empty();
    const prefix: u32 = 'x';
    try expect(!startsWithScalar(whole, prefix));
}

test "startsWithScalar: ascii char" {
    const whole = Str.fromSlice("foobar");
    const prefix: u32 = 'f';
    try expect(startsWithScalar(whole, prefix));
}

test "startsWithScalar: emoji" {
    const yes = Str.fromSlice("💖foobar");
    const no = Str.fromSlice("foobar");
    const prefix: u32 = '💖';
...
test "startsWith: foo starts with fo" {
    const foo = Str.fromSlice("foo");
    const fo = Str.fromSlice("fo");
    try expect(startsWith(foo, fo));
}

test "startsWith: 123456789123456789 starts with 123456789123456789" {
    const str = Str.fromSlice("123456789123456789");
    defer str.decref();
    try expect(startsWith(str, str));
}

test "startsWith: 12345678912345678910 starts with 123456789123456789" {
    const str = Str.fromSlice("12345678912345678910");
    defer str.decref();
    const prefix = Str.fromSlice("123456789123456789");
    defer prefix.decref();
...
// Str.endsWith
pub fn endsWith(string: Str, suffix: Str) callconv(.C) bool {
    const bytes_len = string.len();
    const bytes_ptr = string.asU8ptr();
...
test "endsWith: foo ends with oo" {
    const foo = Str.init("foo", 3);
    const oo = Str.init("oo", 2);
    defer foo.decref();
    defer oo.decref();
...
test "endsWith: 123456789123456789 ends with 123456789123456789" {
    const str = Str.init("123456789123456789", 18);
    defer str.decref();
    try expect(endsWith(str, str));
}

test "endsWith: 12345678912345678910 ends with 345678912345678910" {
    const str = Str.init("12345678912345678910", 20);
    const suffix = Str.init("345678912345678910", 18);
    defer str.decref();
    defer suffix.decref();
...
test "endsWith: hello world ends with world" {
    const str = Str.init("hello world", 11);
    const suffix = Str.init("world", 5);
    defer str.decref();
    defer suffix.decref();
...
// Str.concat
pub fn strConcatC(arg1: Str, arg2: Str) callconv(.C) Str {
    return @call(.always_inline, strConcat, .{ arg1, arg2 });
}

fn strConcat(arg1: Str, arg2: Str) Str {
...
test "Str.concat: small concat small" {
    const str1_len = 3;
    ...
    var roc_str3 = Str.init(str3_ptr, str3_len);
...
pub const ListStr = extern struct {
    list_elements: ?[*]Str,
    list_length: usize,
...
// Str.joinWith
pub fn strJoinWithC(list: roc.List, separator: Str) callconv(.C) Str {
    const roc_list_str = ListStr{
        .list_elements = @as(?[*]Str, @ptrCast(@alignCast(list.bytes))),
...
fn strJoinWith(list: ListStr, separator: Str) Str {
    ...
    const ptr = @as([*]Str, @ptrCast(list.list_elements));
    const slice: []Str = ptr[0..len];
    ...
    var result = Str.allocate(total_size);
...
test "Str.joinWith: result is big" {
    const sep_len = 2;
    var sep: [sep_len]u8 = ", ".*;
    const sep_ptr: [*]u8 = &sep;
    var roc_sep = Str.init(sep_ptr, sep_len);

    const elem_len = 13;
    var elem: [elem_len]u8 = "foobarbazspam".*;
    const elem_ptr: [*]u8 = &elem;
    var roc_elem = Str.init(elem_ptr, elem_len);

    const result_len = 43;
    var xresult: [result_len]u8 = "foobarbazspam, foobarbazspam, foobarbazspam".*;
    const result_ptr: [*]u8 = &xresult;
    var roc_result = Str.init(result_ptr, result_len);

    var elements: [3]Str = .{ roc_elem, roc_elem, roc_elem };
    const list = ListStr{
        .list_length = 3,
        .list_capacity_or_ref_ptr = 3,
        .list_elements = @as([*]Str, @ptrCast(&elements)),
    };
...
// Str.toUtf8
pub fn strToUtf8C(arg: Str) callconv(.C) roc.List {
    return strToBytes(arg);
}

inline fn strToBytes(arg: Str) roc.List {
    const length = arg.len();
    if (length == 0) {
        return roc.List.empty();
    } else if (arg.isSmallStr()) {
        const ptr = utils.allocateWithRefcount(length, Str.alignment);

        @memcpy(ptr[0..length], arg.asU8ptr()[0..length]);

        return roc.List{ .length = length, .bytes = ptr, .capacity_or_ref_ptr = length };
    } else {
        const is_seamless_slice = arg.str_len & SEAMLESS_SLICE_BIT;
        return roc.List{ .length = length, .bytes = arg.str_bytes, .capacity_or_ref_ptr = arg.str_capacity | is_seamless_slice };
    }
}
...
pub fn fromUtf8Range(arg: roc.List, start: usize, count: usize, update_mode: UpdateMode) FromUtf8Result {
    if (arg.len() == 0 or count == 0) {
        arg.decref(Str.alignment);
    ...
    if (isValidUnicode(bytes)) {
        ...
        const string = Str.fromSubListUnsafe(arg, start, count, update_mode);
        return FromUtf8Result{
            .is_ok = true,
            .string = string,
            .byte_index = 0,
...
fn validateUtf8Bytes(bytes: [*]u8, length: usize) FromUtf8Result {
    return fromUtf8Range(roc.List{ .bytes = bytes, .length = length, .capacity_or_ref_ptr = length }, 0, length, .Immutable);
}

fn validateUtf8BytesX(str: roc.List) FromUtf8Result {
...
fn sliceHelp(bytes: [*]const u8, length: usize) roc.List {
    var list = roc.List.allocate(Str.alignment, length, @sizeOf(u8));
...
fn toErrUtf8ByteResponse(index: usize, problem: Utf8ByteProblem) FromUtf8Result {
    return FromUtf8Result{ .is_ok = false, .string = Str.empty(), .byte_index = index, .problem_code = problem };
}
...
fn expectErr(list: roc.List, index: usize, err: Utf8DecodeError, problem: Utf8ByteProblem) !void {
...
pub fn strTrim(input_string: Str) callconv(.C) Str {
    ...
    if (string.isSmallStr()) {
        ...
        return Str.init(string.asU8ptr() + leading_bytes, new_len);
...
pub fn strTrimStart(input_string: Str) callconv(.C) Str {
    var string = input_string;
...
pub fn strTrimEnd(input_string: Str) callconv(.C) Str {
    var string = input_string;
    ...
    if (string.isSmallStr()) {
        // Just create another small string of the correct bytes.
        // No need to decref because it is a small string.
        return Str.init(string.asU8ptr(), new_len);
...
fn countLeadingWhitespaceBytes(string: Str) usize {
...
fn countTrailingWhitespaceBytes(string: Str) usize {
    var byte_count: usize = 0;
...
test "strTrim: empty" {
    const trimmedEmpty = strTrim(Str.empty());
    try expect(trimmedEmpty.eq(Str.empty()));
}

test "strTrim: null byte" {
    const bytes = [_]u8{0};
    const original = Str.init(&bytes, 1);
...
test "strTrim: blank" {
    ...
    const original = Str.init(original_bytes, original_bytes.len);

    const trimmed = strTrim(original);
    defer trimmed.decref();

    try expect(trimmed.eq(Str.empty()));
}
...
test "strTrim: large to large" {
    ...
    const expected = Str.init(expected_bytes, expected_bytes.len);
    defer expected.decref();
...
test "strTrimStart: empty" {
    const trimmedEmpty = strTrimStart(Str.empty());
    try expect(trimmedEmpty.eq(Str.empty()));
}
...
test "strTrimEnd: empty" {
    const trimmedEmpty = strTrimEnd(Str.empty());
    try expect(trimmedEmpty.eq(Str.empty()));
}
...
test "capacity: small string" {
    ...
    var data = Str.init(data_bytes, data_bytes.len);
    defer data.decref();
...
pub fn appendScalar(string: Str, scalar_u32: u32) callconv(.C) Str {
...
test "appendScalar: small A" {
    ...
    const data = Str.init(data_bytes, data_bytes.len);
...
pub fn reserve(string: Str, spare: usize) callconv(.C) Str {
...
pub fn withCapacity(capacity: usize) callconv(.C) Str {
    var str = Str.allocate(capacity);
    str.setLen(0);
    return str;
}

pub fn getScalarUnsafe(string: Str, index: usize) callconv(.C) extern struct { bytesParsed: usize, scalar: u32 } {
...
pub fn strCloneTo(
    ...
    const WIDTH: usize = @sizeOf(Str);
    if (string.isSmallStr()) {
        const array: [@sizeOf(Str)]u8 = @as([@sizeOf(Str)]u8, @bitCast(string));
...
pub fn strReleaseExcessCapacity(
    ...
    if (string.isSmallStr()) {
        ...
        var output = Str.allocateExact(old_length);
