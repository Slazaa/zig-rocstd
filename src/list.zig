...
pub const List = extern struct {
    ...
    pub fn getCapacity(self: List) usize {
    ...
    pub fn isSeamlessSlice(self: List) bool {
    ...
    pub fn seamlessSliceMask(self: List) usize {
    ...
    pub fn empty() List {
        return List{ .bytes = null, .length = 0, .capacity_or_ref_ptr = 0 };
    }
    ...
    pub fn fromSlice(comptime T: type, slice: []const T) List {
        if (slice.len == 0) {
            return List.empty();
        }
    ...
    pub fn getRefcountPtr(self: List) ?[*]u8 {
    ...
    pub fn decref(self: List, alignment: u32) void {
    ...
    pub fn elements(self: List, comptime T: type) ?[*]T {
        return @as(?[*]T, @ptrCast(@alignCast(self.bytes)));
    }

    pub fn isUnique(self: List) bool {
        return self.refcountMachine() == utils.REFCOUNT_ONE;
    }

    fn refcountMachine(self: List) usize {
    ...
    fn refcountHuman(self: List) usize {
        return self.refcountMachine() - utils.REFCOUNT_ONE + 1;
    }

    pub fn makeUniqueExtra(self: List, alignment: u32, element_width: usize, update_mode: UpdateMode) List {
    ...
    pub fn makeUnique(self: List, alignment: u32, element_width: usize) List {
        if (self.isUnique()) {
            return self;
        }
        ...
        const new_list = List.allocate(alignment, self.length, element_width);
        ...
        if (self.bytes) |source_ptr| {
            if (self.isUnique() and !self.isSeamlessSlice()) {
                ...
                if (capacity >= new_length) {
                    return List{ .bytes = self.bytes, .length = new_length, .capacity_or_ref_ptr = capacity };
                } else {
                    ...
                    return List{ .bytes = new_source, .length = new_length, .capacity_or_ref_ptr = new_capacity };
                }
            }
            return self.reallocateFresh(alignment, new_length, element_width);
        }
        return List.allocate(alignment, new_length, element_width);
    }
    ...
    fn reallocateFresh(
        ...
        const result = List.allocate(alignment, new_length, element_width);
...
pub fn listMap(
    ...
    if (list.bytes) |source_ptr| {
        const size = list.len();
        var i: usize = 0;
        const output = List.allocate(alignment, size, new_element_width);
...
fn decrementTail(list: List, start_index: usize, element_width: usize, dec: Dec) void {
...
pub fn listMap2(
    ...
    if (list1.bytes) |source_a| {
        if (list2.bytes) |source_b| {
            const output = List.allocate(alignment, output_length, c_width);
            const target_ptr = output.bytes orelse unreachable;
    ...
    if (list1.bytes) |source_a| {
        if (list2.bytes) |source_b| {
            if (list3.bytes) |source_c| {
                const output = List.allocate(alignment, output_length, d_width);
                const target_ptr = output.bytes orelse unreachable;
        ...
        if (list2.bytes) |source_b| {
            if (list3.bytes) |source_c| {
                if (list4.bytes) |source_d| {
                    const output = List.allocate(alignment, output_length, e_width);
                    const target_ptr = output.bytes orelse unreachable;
...
pub fn listWithCapacity(
    ...
    return listReserve(List.empty(), alignment, capacity, element_width, .InPlace);
}
...
pub fn listReleaseExcessCapacity(
    ...
    if ((update_mode == .InPlace or list.isUnique()) and list.capacity_or_ref_ptr == old_length) {
        ...
        const output = List.allocateExact(alignment, old_length, element_width);
...
fn listAppend(list: List, alignment: u32, element: Opaque, element_width: usize, update_mode: UpdateMode) callconv(.C) List {
...
pub fn listPrepend(list: List, alignment: u32, element: Opaque, element_width: usize) callconv(.C) List {
    const old_length = list.len();
...
pub fn listDropAt(
    ...
    if (list.bytes) |source_ptr| {
        ...
        const output = List.allocate(alignment, size - 1, element_width);
        const target_ptr = output.bytes orelse unreachable;
...
pub fn listConcat(list_a: List, list_b: List, alignment: u32, element_width: usize) callconv(.C) List {
    ...
    const output = List.allocate(alignment, total_length, element_width);
...
pub fn listIsUnique(list: List) callconv(.C) bool {
    return list.isEmpty() or list.isUnique();
}

pub fn listCapacity(list: List) callconv(.C) usize {
    return list.getCapacity();
}

pub fn listRefcountPtr(list: List) callconv(.C) ?[*]u8 {
    return list.getRefcountPtr();
}

test "listConcat: non-unique with unique overlapping" {
    var nonUnique = List.fromSlice(u8, ([_]u8{1})[0..]);
    ...
    var unique = List.fromSlice(u8, ([_]u8{ 2, 3, 4 })[0..]);
    defer unique.decref(@sizeOf(u8));

    var concatted = listConcat(nonUnique, unique, 1, 1);
    var wanted = List.fromSlice(u8, ([_]u8{ 1, 2, 3, 4 })[0..]);
